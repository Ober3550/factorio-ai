...
async function main() {
  ...
  if (argv.length < 1) {
    console.error('Usage: node --loader ts-node/esm tools/transform-blueprint/index.ts <input.json|bp> [--rotate N] [--flip-x] [--flip-y] [--out out.json]')
    process.exit(2)
  }
  ...
  try {
    ...
    const args = ['--no-warnings', '--loader', 'ts-node/esm', 'tools/blueprint-ascii/index.ts', dest, '--zoom-2']
